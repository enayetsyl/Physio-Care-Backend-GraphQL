...
describe("Auth Resolver Integration Tests", () => {
  describe("sendOTP mutation", () => {
    it("should create and return OTP for valid mobile", async () => {
      ...
      const result = await authResolver.Mutation.sendOTP(null, { mobile });
    ...
    it("should invalidate previous OTPs when creating new one", async () => {
      ...
      const firstOTP = await OTP.findOne({ mobile, verified: false });

      // Create second OTP
      await authResolver.Mutation.sendOTP(null, { mobile });

      // First OTP should be invalidated
      const firstOTPAfter = await OTP.findById(firstOTP?._id);
      expect(firstOTPAfter?.verified).toBe(true);

      // New OTP should exist
      const newOTP = await OTP.findOne({ mobile, verified: false });
    ...
    it("should throw validation error for invalid mobile", async () => {
      ...
      await expect(
        authResolver.Mutation.sendOTP(null, { mobile: invalidMobile })
      ).rejects.toThrow();
    });
  });
  ...
  describe("verifyOTP mutation", () => {
    it("should verify OTP and return token for existing user", async () => {
      ...
      // Verify OTP
      const result = await authResolver.Mutation.verifyOTP(null, {
        mobile,
        otp,
      });
    ...
    it("should create new user and return token when user doesn't exist", async () => {
      ...
      const createdUser = await User.findOne({ mobile });
    ...
    it("should throw error when user doesn't exist and no userDetails provided", async () => {
      ...
      await expect(
        authResolver.Mutation.verifyOTP(null, {
          mobile,
          otp,
        })
  ...
  describe("refreshToken mutation", () => {
    it("should refresh token for authenticated user", async () => {
      ...
      const result = await authResolver.Mutation.refreshToken(
        null,
        null,
        context
      );
    ...
    it("should throw error when user is not authenticated", async () => {
      ...
      await expect(
        authResolver.Mutation.refreshToken(null, null, context)
  ...
  describe("OTP attempt limits", () => {
    it("should increment attempts on wrong OTP", async () => {
      ...
      const correctOTP = await OTPService.createOTP(mobile);

      // Try wrong OTP multiple times
      for (let i = 0; i < 3; i++) {
        try {
          await authResolver.Mutation.verifyOTP(null, {
            mobile,
            otp: "000000",
          });
        } catch (error) {
      ...
      const newOTP = await OTPService.createOTP(mobile);
